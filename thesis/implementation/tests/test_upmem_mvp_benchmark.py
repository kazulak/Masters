...
def test_upmem_mvp_benchmark_records_same_route_quantization_comparison(monkeypatch, tmp_path: Path) -> None:
    ...
    assert all(record["full_precision_reference_kind"] == "cpu_exact_taskgraph_full_precision" for record in upmem_records)
    assert all(record["full_precision_max_abs_error"] is not None for record in upmem_records)
    assert all(record["total_host_orchestration_time_s"] is None for record in upmem_records)
    assert all(record["host_residual_time_kind"] == "derived_wall_minus_simulator" for record in upmem_records)
    assert all(record["total_bridge_time_s"] is not None for record in upmem_records)
    assert all(record["total_build_time_s"] is not None for record in upmem_records)
    assert all(record["quantization_clipping_count"] is not None for record in upmem_records)
    assert all(record["quantization_saturation_count"] is not None for record in upmem_records)
    assert all(record["complex_split_complex_task_count"] == 0 for record in upmem_records)
