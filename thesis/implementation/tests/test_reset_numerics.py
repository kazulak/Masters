...
def _encode_pair(left: np.ndarray, right: np.ndarray, policy: NumericPolicy):
    return (
        encode_complex_tensor(left, policy),
        encode_complex_tensor(right, policy),
    )
...
def test_float_encoding_rejects_nonfinite_values(value: np.ndarray) -> None:
...
def test_float_encoding_handles_zero_real_and_imaginary_inputs() -> None:
    zero = encode_complex_tensor(np.zeros((2, 2), dtype=np.complex128), FLOAT)
    real = encode_complex_tensor(np.array([1.0, -2.0], dtype=np.float64), FLOAT)
    imag = encode_complex_tensor(1j * np.array([1.0, -2.0]), FLOAT)

    for encoded in (zero, real, imag):
        assert encoded.real.dtype == np.float32
        assert encoded.imag.dtype == np.float32
        assert encoded.scale == 1.0
        assert encoded.saturation_real == 0
        assert encoded.saturation_imag == 0
    np.testing.assert_array_equal(real.imag, 0.0)
    np.testing.assert_array_equal(imag.real, 0.0)
    np.testing.assert_array_equal(imag.imag, [1.0, -2.0])


def test_int8_uses_one_shared_scale_and_zero_fallback() -> None:
    value = np.array([1.0 + 4.0j, -2.0 - 1.0j])
    encoded = encode_complex_tensor(value, INT8)
    assert encoded.scale == pytest.approx(4.0 / 127.0)
    assert encoded.real.dtype == np.int8
    assert encoded.imag.dtype == np.int8
...
def test_int8_rounding_is_nearest_even_and_encoding_is_detached_readonly() -> None:
    source = np.array([0.5 + 0j, 1.5 + 0j, -0.5 + 0j, -1.5 + 0j, 127.0 + 0j])
    encoded = encode_complex_tensor(source, INT8)
    assert encoded.scale == 1.0
    np.testing.assert_array_equal(encoded.real, [0, 2, 0, -2, 127])
    ...
    source[0] = 99.0
    assert encoded.real[0] == 0
    with pytest.raises(ValueError):
        encoded.real[0] = 1


def test_float_products_follow_arbitrary_labels_and_explicit_formula() -> None:
    node = _matrix_node()
    ...
    for actual, expected_product in zip(products, expected_products):
        np.testing.assert_array_equal(actual, expected_product)
    expected = np.einsum("ik,kj->ij", left.astype(np.complex64), right.astype(np.complex64))
    decoded = decode_complex_products(products, 1.0, 1.0, FLOAT)
    np.testing.assert_allclose(decoded, expected.astype(np.complex64), rtol=0, atol=0)
    assert all(product.dtype == np.float32 for product in products)
    assert all(product.flags.owndata and not product.flags.writeable for product in products)


def test_float_random_decode_matches_float32_four_product_formula() -> None:
    rng = np.random.default_rng(7)
    left = (rng.normal(size=(2, 4)) + 1j * rng.normal(size=(2, 4))).astype(np.complex64)
    right = (rng.normal(size=(4, 3)) + 1j * rng.normal(size=(4, 3))).astype(np.complex64)
    products = contract_complex_products(
        ...
        *(_encode_pair(left, right, FLOAT)),
        FLOAT,
    )
    expected_real = np.einsum("ik,kj->ij", left.real, right.real, dtype=np.float32) - np.einsum(
        "ik,kj->ij", left.imag, right.imag, dtype=np.float32
    )
    expected_imag = np.einsum("ik,kj->ij", left.real, right.imag, dtype=np.float32) + np.einsum(
        "ik,kj->ij", left.imag, right.real, dtype=np.float32
    )
    expected = expected_real.astype(np.complex64) + 1j * expected_imag.astype(np.complex64)
    np.testing.assert_allclose(decode_complex_products(products, 1.0, 1.0, FLOAT), expected, atol=0, rtol=0)


def test_int8_products_have_int32_dtype_decode_and_are_deterministic() -> None:
    node = _matrix_node()
    ...
    assert all(product.dtype == np.int32 for product in products)
...
def test_outer_product_preserves_requested_output_order() -> None:
    ...
    expected = right[:, None] * left[None, :]
    np.testing.assert_array_equal(decode_complex_products(products, 1.0, 1.0, FLOAT), expected)


def test_one_sided_contracted_label_is_reduced() -> None:
    node = ContractNode(
        node_id="one_sided",
        left=TensorView(tensor_id="left", labels=(0, 1), shape=(2, 3)),
        right=TensorView(tensor_id="right", labels=(2,), shape=(4,)),
        output=TensorSpec(id="out", labels=(0, 2), shape=(2, 4), structure="dense"),
        contracted_labels=(1,),
        output_labels=(0, 2),
    )
    left = np.arange(6, dtype=np.float32).reshape(2, 3).astype(np.complex64)
    right = np.arange(4, dtype=np.float32).astype(np.complex64)
    products = contract_complex_products(node, *_encode_pair(left, right, FLOAT), FLOAT)
    expected = left.sum(axis=1)[:, None] * right[None, :]
    np.testing.assert_array_equal(decode_complex_products(products, 1.0, 1.0, FLOAT), expected)


def test_scalar_output_is_supported() -> None:
    node = ContractNode(
        node_id="scalar",
        left=TensorView(tensor_id="left", labels=(0,), shape=(3,)),
        right=TensorView(tensor_id="right", labels=(0,), shape=(3,)),
        output=TensorSpec(id="out", labels=(), shape=(), structure="dense"),
        contracted_labels=(0,),
        output_labels=(),
    )
    left = np.array([1.0 + 1j, 2.0, 3.0 - 1j])
    right = np.array([4.0, 5.0 + 1j, 6.0])
    products = contract_complex_products(node, *_encode_pair(left, right, FLOAT), FLOAT)
    decoded = decode_complex_products(products, 1.0, 1.0, FLOAT)
    assert decoded.shape == ()
    np.testing.assert_array_equal(decoded, np.einsum("i,i->", left.astype(np.complex64), right.astype(np.complex64)))


def test_contract_rejects_unsafe_k_before_einsum() -> None:
    ...
    left_safe = EncodedComplexTensor(np.zeros((1, safe_k), dtype=np.int8), np.zeros((1, safe_k), dtype=np.int8), 1.0, 0, 0)
    right_safe = EncodedComplexTensor(np.zeros((safe_k, 1), dtype=np.int8), np.zeros((safe_k, 1), dtype=np.int8), 1.0, 0, 0)
    safe_products = contract_complex_products(safe, left_safe, right_safe, INT8)
    assert safe_products[0].dtype == np.int32
    np.testing.assert_array_equal(safe_products[0], np.zeros((1, 1), dtype=np.int32))

    unsafe = node_with_k(safe_k + 1)
    left = EncodedComplexTensor(np.zeros((1, safe_k + 1), dtype=np.int8), np.zeros((1, safe_k + 1), dtype=np.int8), 1.0, 0, 0)
    right = EncodedComplexTensor(np.zeros((safe_k + 1, 1), dtype=np.int8), np.zeros((safe_k + 1, 1), dtype=np.int8), 1.0, 0, 0)
    with pytest.raises(ValueError, match="int32 accumulation"):
        contract_complex_products(unsafe, left, right, INT8)


def test_malformed_encoded_values_and_output_descriptors_are_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="within plane size"):
        EncodedComplexTensor(np.ones(2, dtype=np.int8), np.ones(2, dtype=np.int8), 1.0, 3, 0)
    ...
    bad_output = replace(
        node,
        output=TensorSpec(id="out", labels=(41, 73), shape=(3, 2), structure="dense"),
    )
    with pytest.raises(ValueError, match="output descriptor shape"):
        contract_complex_products(bad_output, left, right, FLOAT)
    bad_labels = replace(node, output_labels=(41,), output=TensorSpec(id="out", labels=(41,), shape=(2,), structure="dense"))
    with pytest.raises(ValueError, match="cover all input labels"):
        contract_complex_products(bad_labels, left, right, FLOAT)


def test_float_products_reject_nonfinite_results() -> None:
    node = _matrix_node(left_shape=(1, 1), right_shape=(1, 1), contracted_labels=(900,), output_labels=(41, 73))
    left = EncodedComplexTensor(np.array([[np.finfo(np.float32).max]], dtype=np.float32), np.zeros((1, 1), dtype=np.float32), 1.0, 0, 0)
    right = EncodedComplexTensor(np.array([[np.finfo(np.float32).max]], dtype=np.float32), np.zeros((1, 1), dtype=np.float32), 1.0, 0, 0)
    with pytest.raises(ValueError, match="nonfinite"):
        contract_complex_products(node, left, right, FLOAT)


def test_vectorized_int64_addition_overflow_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="overflow"):
        decode_complex_products(products, 1.0, 1.0, INT8)


def test_malformed_shapes_dtypes_scales_and_int64_overflow_are_rejected() -> None:
    node = _matrix_node()
    left, right = _encode_pair(np.ones((2, 3), dtype=np.complex64), np.ones((3, 2), dtype=np.complex64), FLOAT)
    malformed = EncodedComplexTensor(np.ones((1, 1), dtype=np.float32), np.ones((1, 1), dtype=np.float32), 1.0, 0, 0)
    with pytest.raises(ValueError, match="shape"):
        contract_complex_products(node, malformed, right, FLOAT)
    wrong_dtype = EncodedComplexTensor(np.ones((2, 3), dtype=np.int8), np.ones((2, 3), dtype=np.int8), 1.0, 0, 0)
    with pytest.raises(ValueError, match="dtype"):
        contract_complex_products(node, wrong_dtype, right, FLOAT)
    ...
    with pytest.raises(ValueError, match="overflow"):
        decode_complex_products(int_products, 1.0, 1.0, INT8)
