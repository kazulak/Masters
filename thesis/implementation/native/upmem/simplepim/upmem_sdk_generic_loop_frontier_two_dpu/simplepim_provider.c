...
static void free_management_metadata(simplepim_management_t *management) {
    ...
    /* M4.1 registers no SimplePIM tables. Do not guess ownership for a
     * future table route; that route needs its own cleanup contract. */
...
dpu_error_t simplepim_frontier_provider_init(
    ...
    if (provider == NULL || requested_dpus != 2u || allocation_profile == NULL ||
        initialization_binary == NULL) {
        return DPU_ERR_INVALID_PROFILE;
    }
    *provider = (simplepim_frontier_provider_t){0};
    provider->allocation_attempted = 1;
    if (strcmp(allocation_profile, "backend=hw") != 0) return DPU_ERR_INVALID_PROFILE;
    ...
    if (error != DPU_OK || provider->observed_dpus != requested_dpus) {
        if (error == DPU_OK) error = DPU_ERR_INVALID_PROFILE;
        return error;
    }
...
dpu_error_t simplepim_frontier_provider_release(simplepim_frontier_provider_t *provider) {
    ...
    free_management_metadata(provider->management);
