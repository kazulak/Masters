...
static void v3_write_response(
    ...
    fprintf(file, ",\"target_requested\":\"hardware\",\"target_observed\":\"%s\",\"backend_id\":\"upmem_sdk_hardware_execution_plan_v3\",\"backend_family\":\"upmem_sdk\",\"execution_class\":\"resident_taskgraph\",\"kernel_strategy\":\"resident_generic_contract\",\"requested_dpu_count\":%u,\"allocated_dpu_count\":%u,\"requested_rank_path\":", status != NULL && strcmp(status, "completed") == 0 ? "physical_hardware" : "not_allocated", dpu_count, provider != NULL && provider->allocation_used ? provider->observed_dpus : 0u);
