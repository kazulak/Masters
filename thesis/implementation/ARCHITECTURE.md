...
|---|---|---|
| `cpu_tn_einsum_exact` | Internal debug/small exact tensor-network route | Comparable final tensor output for bounded diagnostics |
| `quimb_tn_exact` | Serious external exact tensor-network CPU baseline | Comparable final tensor output with Quimb/cotengra provenance |
| `quest_cpu_full_state_benchmark` | QuEST CPU full-state baseline | Benchmark-only metrics |
| `quest_cpu_full_state_exact` | Serious QuEST CPU full-state comparable baseline | Statevector output for small deterministic unitary circuits |
...
backend. It is exact CPU execution, not GPU execution, and is the serious CPU
TN baseline for compute-focused and manual thesis evidence. The simple internal
`cpu_tn_einsum_exact` route remains useful for small correctness/debug work, but
its NumPy einsum symbol/lowering limits can reject index-heavy circuits. Those
rejections document an internal engine limitation, not a tensor-network
approach limitation.
...
| CPU exact task execution | `providers/exact_tn/cpu_einsum.py` | Implemented internal debug/small route |
| External exact TN execution | `providers/exact_tn/quimb_tn.py` | Implemented serious Quimb/cotengra-backed baseline |
...
| `simulation-backend-compare` | QuEST full-state, serious external TN, and optional internal TN diagnostics on shared deterministic circuits | CPU backend comparison scaffold |
...
Simulation backend comparison has five suite tiers. `simulation_backend_compare_quick.yml`
...
GPU execution belongs in `simulation_backend_compare_gpu_medium.yml` only after
a route proves real GPU computation; otherwise GPU candidates stay in probe metadata.
In compute-focused and manual suites, `quimb_tn_exact` is the primary TN
baseline. `cpu_tn_einsum_exact` is labeled as an internal debug/diagnostic
route so its failures do not read as failures of the TN method.
