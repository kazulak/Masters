...
| Strict UPMEM runtime | `thesis/implementation/src/quantum_bench/targets/upmem/taskgraph_runtime.py`, `numeric_reference.py`, `runtime_evidence.py` | Execute policy/scheduling while keeping CPU references, validation, and evidence construction reviewable | Active, SDK simulator & physical hardware |
| Physical UPMEM qualification lanes | M2 sliced-resident, M3.1 frontier, M4.2–M4.6 resident routes under `bench/`, `targets/upmem/`, and `native/upmem/simplepim/` | Bounded physical functionality, multi-tasklet tiling, descriptor-driven runtime, and timing instrumentation | **M4 Complete.** Passed physical campaign on ETH (336 measurement rows across 13 cases, 0 failures, $1 \le NR\_TASKLETS \le 16$). |
