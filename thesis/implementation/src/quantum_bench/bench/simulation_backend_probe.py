...
QUEST_GPU_VERIFICATION_ARTIFACT = (
    Path("build") / "gpu_verification" / "quest_gpu_full_state_exact.json"
)
...
def probe_simulation_backends(
    root_dir: Path, *, verify_gpu: str = "none", verify_gpu_tn: str = "none"
) -> JsonDict:
    ...
    if verify_gpu != "none":
        gpu_verification = _with_verification_source(
            _verify_gpu_backend(root_dir, verify_gpu, gpu_hardware), "fresh_verify_gpu"
        )
    else:
        gpu_verification = _with_verification_source(
            _load_quest_gpu_verification(root_dir), "cached_artifact"
        )
    if verify_gpu_tn != "none":
        gpu_tn_verification = _with_verification_source(
            _verify_gpu_tn_candidate(root_dir, verify_gpu_tn, gpu_hardware),
            "fresh_verify_gpu_tn",
        )
    else:
    ...
    gpu_tn_candidates = _gpu_tn_candidate_matrix(
        gpu_candidates, gpu_hardware, gpu_tn_verification
    )
    payload = {
        ...
        "gpu_probe": {
            ...
            "gpu_tensor_network_probe": {
                ...
                "gpu_tn_verification": _gpu_tn_verification_probe_summary(
                    gpu_tn_verification
                ),
            },
            "gpu_verification": _gpu_verification_probe_summary(gpu_verification),
            "gpu_verification_source": gpu_verification.get("verification_source")
            if gpu_verification
            else "none",
            "cached_gpu_verification_used": bool(
                gpu_verification
                and gpu_verification.get("verification_source") == "cached_artifact"
            ),
            "cuda_only_assumption_used": False,
            "gpu_execution_backend_added": any(
                bool(candidate["benchmark_route_eligible"])
...
def _gpu_verification_probe_summary(payload: JsonDict | None) -> JsonDict | None:
    ...
    if "attempted_steps" in payload:
        summary["attempted_steps"] = [
            {key: step.get(key) for key in ("step", "status", "command") if key in step}
...
def _gpu_tn_verification_probe_summary(payload: JsonDict | None) -> JsonDict | None:
    ...
    if "attempted_steps" in payload:
        summary["attempted_steps"] = [
            {key: step.get(key) for key in ("step", "status", "reason") if key in step}
            for step in (payload.get("attempted_steps") or [])
            if isinstance(step, dict)
        ]
    if "detected_dependencies" in payload:
...
def _gpu_hardware_probe() -> JsonDict:
    ...
    amd = [
        ...
        if any(marker in line.lower() for marker in ("amd", "ati", "radeon"))
    ]
...
def _lspci_gpu_lines() -> list[str]:
    try:
        result = subprocess.run(
            ["lspci"], check=False, capture_output=True, text=True, timeout=3
        )
    except Exception:
        return []
    ...
    for line in result.stdout.splitlines():
        ...
        if any(
            marker in lowered
            for marker in ("vga", "3d", "display", "radeon", "nvidia", "amd/ati")
        ):
...
def _gpu_candidate_matrix(
    root_dir: Path, hardware: JsonDict, verification: JsonDict | None = None
) -> list[JsonDict]:
    quest_cmake = root_dir / "external" / "QuEST" / "CMakeLists.txt"
    quest_text = (
        quest_cmake.read_text(encoding="utf-8", errors="ignore")
        if quest_cmake.exists()
        else ""
    )
    quest_has_hip = "ENABLE_HIP" in quest_text
    quest_has_cuda = "ENABLE_CUDA" in quest_text
    rocm_tools = {
        name: _command_status(name) for name in ("hipcc", "rocminfo", "rocm-smi")
    }
    cuda_tools = {name: _command_status(name) for name in ("nvcc", "nvidia-smi")}
    torch_gpu = _torch_gpu_execution_status()
    verification_source = (
        str(verification.get("verification_source") or "fresh_verify_gpu")
        if verification
        else "none"
    )
    hip_verified = bool(
        verification
        and verification.get("selected_backend") == "quest-hip"
        and verification.get("status") == "verified"
    )
    cuda_verified = bool(
        verification
        and verification.get("selected_backend") == "quest-cuda"
        and verification.get("status") == "verified"
    )
    hip_blocker = (
        verification.get("blocker_reason")
        if verification and verification.get("selected_backend") == "quest-hip"
        else None
    )
    cuda_blocker = (
        verification.get("blocker_reason")
        if verification and verification.get("selected_backend") == "quest-cuda"
        else None
    )
    hip_preflight_available = _quest_hip_live_preflight_available(hardware, rocm_tools)
    cuda_preflight_available = _quest_cuda_live_preflight_available(
        hardware, cuda_tools
    )
    hip_classification = _verified_gpu_candidate_classification(
        ...
        default_classification="amd_rocm_candidate_not_verified"
        if quest_has_hip
        else "feasible_later_not_benchmarkable_now",
    )
    cuda_classification = _verified_gpu_candidate_classification(
        ...
        default_classification="nvidia_cuda_only_not_usable_here"
        if not hardware.get("nvidia_gpu_pci_detected")
        else "feasible_later_not_benchmarkable_now",
    )
    candidates = [
        _candidate(
            ...
            if hip_verified
                ...
                hip_blocker
                or (
                    "quest_enable_hip_source_only_not_benchmark_evidence"
                    if quest_has_hip
                    else "quest_enable_hip_not_detected"
                )
            ),
            ...
            extra={
                ...
                "verification_source": verification_source
                if hip_verified or hip_blocker
                else "none",
                "current_process_gpu_preflight_available": hip_preflight_available,
                "cached_verification_artifact_used": bool(
                    hip_verified and verification_source == "cached_artifact"
                ),
            },
        ),
        _candidate(
            ...
            if cuda_verified
                ...
                cuda_blocker or "requires_nvidia_cuda_build_and_minimal_gpu_execution"
            ),
            ...
            extra={
                ...
                "verification_source": verification_source
                if cuda_verified or cuda_blocker
                else "none",
                "current_process_gpu_preflight_available": cuda_preflight_available,
                "cached_verification_artifact_used": bool(
                    cuda_verified and verification_source == "cached_artifact"
                ),
            },
        ),
        _python_candidate(
            "qiskit_aer_gpu_statevector",
            "cuda_quantum_stack",
            3,
            "nvidia_cuda",
            "qiskit_aer",
            hardware,
            "qiskit_aer_gpu_execution_not_verified",
        ),
        _python_candidate(
            "qiskit_aer_gpu_tensor_network",
            "cuda_quantum_stack",
            4,
            "nvidia_cuda",
            "qiskit_aer",
            hardware,
            "qiskit_aer_cuquantum_execution_not_verified",
        ),
        _python_candidate(
            "cudaq_gpu_full_state",
            "cuda_quantum_stack",
            5,
            "nvidia_cuda",
            "cudaq",
            hardware,
            "cudaq_gpu_execution_not_verified",
        ),
        _python_candidate(
            "cudaq_gpu_tensor_network",
            "cuda_quantum_stack",
            6,
            "nvidia_cuda",
            "cudaq",
            hardware,
            "cudaq_tn_gpu_execution_not_verified",
        ),
        _python_candidate(
            ...
            "cuquantum",
            hardware,
            "cuquantum_integration_not_implemented",
        ),
        _python_candidate(
            "cuquantum_tensor_network",
            "cuda_quantum_stack",
            8,
            "nvidia_cuda",
            "cuquantum",
            hardware,
            "cutensornet_integration_not_implemented",
        ),
...
def _quest_hip_live_preflight_available(
    hardware: JsonDict, rocm_tools: JsonDict
) -> bool:
    return bool(
        ...
        and (
            hardware.get("dev_dri_render_node_present")
            or hardware.get("dev_dri_renderD128_present")
        )
...
def _quest_cuda_live_preflight_available(
    hardware: JsonDict, cuda_tools: JsonDict
) -> bool:
    return bool(
...
def _verified_gpu_candidate_classification(
    *,
    ...
    if (
        ...
        and not current_process_preflight_available
    ):
...
def _gpu_tn_candidate_matrix(
    ...
    if verification:
        candidates = [
            _annotate_gpu_tn_candidate(candidate, verification)
...
def _annotate_gpu_tn_candidate(candidate: JsonDict, verification: JsonDict) -> JsonDict:
    ...
    generic_verified = bool(
        verification.get("minimal_gpu_tensor_contraction_verified", False)
    )
    status = str(verification.get("status") or "")
    annotated = dict(candidate)
    annotated.update(
        {
            "classification": "minimal_execution_verified"
            if verified or generic_verified
            else candidate.get("classification"),
            "blocker_reason": None
            if verified or generic_verified
            else verification.get("blocker_reason") or candidate.get("blocker_reason"),
            "gpu_execution_verified": bool(
                verification.get("gpu_program_executed", False)
            ),
            ...
            "route_status": "verified_feasibility_only"
            if status == "verified"
            else candidate.get("route_status"),
            "verification_artifact": verification.get("artifact_path"),
            "verification_source": verification.get(
                "verification_source", "fresh_verify_gpu_tn"
            ),
            ...
            "minimal_tn_validation_status": verification.get(
                "minimal_tn_validation_status"
            ),
            "cpu_fallback_used": bool(verification.get("cpu_fallback_used", False)),
            "cpu_fallback_detected": bool(
                verification.get("cpu_fallback_detected", False)
            ),
...
def _gpu_tn_candidate_from_gpu_candidate(
    ...
    *,
    ...
    return {
        ...
        "blocker_reason": source.get(
            "blocker_reason", f"{source_candidate_id}_not_available"
        ),
...
def _verify_gpu_tn_candidate(
    root_dir: Path, requested_candidate: str, hardware: JsonDict
) -> JsonDict:
    ...
    if selected is None:
        return _write_gpu_tn_verification_artifact(
            root_dir,
            {
                ...
                "attempted_steps": [
                    {"step": "select_gpu_tn_candidate", "status": "blocked"}
                ],
                "benchmark_route_eligible": False,
                "benchmark_records_emitted": False,
            },
    ...
    if selected == "cuquantum_cutensornet":
        ...
        result = _blocked_gpu_tn_execution(
            selected,
            selected,
            "unknown_gpu_tn_candidate",
            target_gpu_stack="unknown",
            backend_family=selected,
        )
    return _write_gpu_tn_verification_artifact(
        root_dir,
        {
...
def _select_gpu_tn_candidate(
    requested_candidate: str, hardware: JsonDict
) -> str | None:
    ...
    try:
        ...
        return _gpu_tn_execution_payload(
            ...
            attempted_steps=[
                ...
                {
                    "step": "minimal_cutensornet_contract",
...
def _write_gpu_tn_verification_artifact(root_dir: Path, payload: JsonDict) -> JsonDict:
    selected = str(
        ...
        or payload.get("requested_candidate")
        or "gpu_tn"
    )
    ...
    path.write_text(
        json.dumps(to_jsonable(final_payload), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
...
def _verify_gpu_backend(
    root_dir: Path, requested_backend: str, hardware: JsonDict
) -> JsonDict:
    ...
    if selected_backend == "torch-rocm":
        ...
        return _write_gpu_verification_artifact(
            root_dir,
            {
                ...
                "attempted_steps": [
                    {
                        "step": "torch_rocm_minimal_tensor_execution",
    ...
    attempted_steps = [
        {
            "step": "preflight",
            "status": "passed" if not prereq["missing"] else "blocked",
        }
    ]
    if prereq["missing"]:
        return _write_gpu_verification_artifact(
    ...
    runner_bin_dir = (
        ...
        / "native"
        / "quest_gpu"
        / ("hip" if selected_backend == "quest-hip" else "cuda")
        / "bin"
    )
    ...
    build_cmd = [
        ...
        "clean-all",
        "all",
        f"GPU_BACKEND={'hip' if selected_backend == 'quest-hip' else 'cuda'}",
    ]
    ...
    if selected_backend == "quest-hip":
        ...
        smoke_build_result = _run_command(
            smoke_build_cmd, cwd=runner_root, timeout_s=120
        )
        attempted_steps.append(
            {
        ...
        smoke_sha256_before = _file_sha256(smoke)
        if smoke_sha256_before is None:
            return _write_gpu_verification_artifact(
                root_dir,
                {
                    "status": "failed",
                    ...
                    "blocker_reason": "hip_smoke_hash_before_failed",
                    "detected_dependencies": prereq["dependencies"],
                    "attempted_steps": attempted_steps,
                    "hip_smoke_build_result": smoke_build_result,
                    "runtime_probe_path": str(smoke),
                    "runtime_probe_sha256_before": None,
                },
            )
        smoke_run_result = _run_command(smoke_run_cmd, cwd=runner_root, timeout_s=30)
        smoke_sha256_after = _file_sha256(smoke)
        ...
        smoke_verified = bool(
            ...
            and hip_smoke_payload.get("gpu_backend_verified") is True
            and hip_smoke_payload.get("gpu_synchronized") is True
            and isinstance(hip_smoke_payload.get("device_count"), int)
            and not isinstance(hip_smoke_payload.get("device_count"), bool)
            and hip_smoke_payload.get("device_count", 0) > 0
            and isinstance(hip_smoke_payload.get("gpu_device_name"), str)
            and bool(hip_smoke_payload.get("gpu_device_name", "").strip())
            and smoke_sha256_after == smoke_sha256_before
        )
        attempted_steps.append(
            {
        ...
        if not smoke_verified:
            return _write_gpu_verification_artifact(
                root_dir,
                {
                    ...
                    "gpu_program_executed": False,
                    "gpu_synchronized": False,
                    "gpu_device_name": _gpu_device_name_from_hip_smoke(
                        hip_smoke_payload
                    ),
                    ...
                    "runtime_probe_path": str(smoke),
                    "runtime_probe_sha256_before": smoke_sha256_before,
                    "runtime_probe_sha256_after": smoke_sha256_after,
                },
            )

    dump_path = (
        root_dir / "build" / "gpu_verification" / "quest_gpu_minimal_state_dump.json"
    )
    run_cmd = [
        str(runner),
        "--algo",
    ...
    runner_sha256_before = _file_sha256(runner)
    if runner_sha256_before is None:
        return _write_gpu_verification_artifact(
            root_dir,
            {
                "status": "failed",
                ...
                "runner_sha256_before": None,
                "cpu_fallback_used": False,
                "blocker_reason": "quest_gpu_runner_hash_before_failed",
                "detected_dependencies": prereq["dependencies"],
                "attempted_steps": attempted_steps,
                "hip_smoke_payload": hip_smoke_payload,
                "build_result": build_result,
    ...
    runner_sha256_after = _file_sha256(runner)
    attempted_steps.append(
        {
            "step": "minimal_quest_gpu_run",
            "status": "passed" if run_result["returncode"] == 0 else "failed",
            "command": run_cmd,
        }
    )
    payload = _json_from_stdout(run_result.get("stdout") or "")
    runner_sha256_final = _file_sha256(runner)
    smoke_sha256_final = (
        _file_sha256(smoke) if selected_backend == "quest-hip" else None
    )
    binaries_stable = bool(
        runner_sha256_before == runner_sha256_after == runner_sha256_final
        and (
            selected_backend != "quest-hip"
            or smoke_sha256_before == smoke_sha256_after == smoke_sha256_final
        )
    )
    verified = bool(
        run_result["returncode"] == 0
        and payload
        and payload.get("status") in {"ok", "passed"}
        and dump_path.exists()
        and binaries_stable
        and (
            selected_backend != "quest-hip"
            or (hip_smoke_payload and hip_smoke_payload.get("gpu_synchronized") is True)
        )
    )
    artifact_updates = {
        "runner_path": str(runner),
        "runner_root": str(runner_root),
        "runner_sha256_before": runner_sha256_before,
        "runner_sha256_after": runner_sha256_after,
        "runner_sha256_final": runner_sha256_final,
        "cpu_fallback_used": False,
    }
    if selected_backend == "quest-hip":
        artifact_updates.update(
            {
                "runtime_probe_path": str(runner_bin_dir / "hip_smoke"),
                "runtime_probe_sha256_before": smoke_sha256_before,
                "runtime_probe_sha256_after": smoke_sha256_after,
                "runtime_probe_sha256_final": smoke_sha256_final,
            }
        )
    if selected_backend == "quest-hip" and verified:
        artifact_updates.update(
            {
                "runner_sha256": runner_sha256_before,
                "runtime_probe_sha256": smoke_sha256_before,
    ...
    return _write_gpu_verification_artifact(
        root_dir,
        {
            ...
            "gpu_device_count": (
                hip_smoke_payload.get("device_count") if hip_smoke_payload else None
            ),
            ...
            **artifact_updates,
            "blocker_reason": (
                ...
                if verified
                    ...
                    "qualified_binary_missing_or_changed"
                    if not binaries_stable
                    else "quest_gpu_minimal_run_failed"
                )
            ),
            "detected_dependencies": prereq["dependencies"],
            "attempted_steps": attempted_steps,
...
def _quest_gpu_prerequisites(selected_backend: str, hardware: JsonDict) -> JsonDict:
    if selected_backend == "quest-hip":
        checks = {
            ...
            "dev_dri_renderD128": {
                "available": bool(hardware.get("dev_dri_renderD128_present")),
                "required": False,
            },
            "dev_dri_render_node": {
                "available": bool(
        ...
        checks = {
            "nvidia_gpu_pci": {
                "available": bool(hardware.get("nvidia_gpu_pci_detected"))
            },
            "nvidia-smi": _command_status("nvidia-smi"),
            "nvcc": _command_status("nvcc"),
            "cmake": _command_status("cmake"),
...
def _write_gpu_verification_artifact(
    root_dir: Path, payload: JsonDict, *, artifact_name: str | None = None
) -> JsonDict:
    path = (
        _quest_gpu_verification_path(root_dir)
        if artifact_name is None
        else root_dir / "build" / "gpu_verification" / artifact_name
    )
    path.parent.mkdir(parents=True, exist_ok=True)
    final_payload = {
...
def _quest_gpu_verification_path(root_dir: Path) -> Path:
    return root_dir / QUEST_GPU_VERIFICATION_ARTIFACT


def _load_quest_gpu_verification(root_dir: Path) -> JsonDict | None:
    path = _quest_gpu_verification_path(root_dir)
    if not path.exists():
        return None
    try:
...
def _gpu_device_name(selected_backend: str) -> str | None:
    if selected_backend == "quest-hip":
        ...
        if result.get("returncode") == 0 and result.get("stdout"):
            ...
            gfx = next(
                (
                    line.split("GFX Version:", 1)[1].strip()
                    for line in lines
...
def _candidate(
    ...
    *,
    ...
    return {
        ...
        if usable_current and gpu_execution_verified
        else classification,
        "benchmark_route_eligible": bool(
            gpu_execution_verified
            if benchmark_route_eligible is None
            else benchmark_route_eligible
        ),
...
def _quimb_gpu_candidate(hardware: JsonDict) -> JsonDict:
    ...
    if not deps["quimb"]["available"] or not deps["cotengra"]["available"]:
        ...
        hardware.get("amd_gpu_pci_detected") or hardware.get("nvidia_gpu_pci_detected")
    ):
        classification = "feasible_later_not_benchmarkable_now"
...
def _torch_gpu_execution_status() -> JsonDict:
    ...
    try:
        ...
        return {
            "classification": "usable_current_machine"
            if verified
            else "amd_rocm_candidate_not_verified",
            "reason": None
            if verified
            else "torch_tensor_execution_not_verified_on_gpu",
