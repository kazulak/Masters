...
def label_count(label_sets: list[tuple[int, ...]], output_labels: tuple[int, ...]) -> int:
    return len({label for labels in label_sets for label in labels} | set(output_labels))


def supports_string_einsum(label_sets: list[tuple[int, ...]], output_labels: tuple[int, ...]) -> bool:
    return label_count(label_sets, output_labels) <= len(_SYMBOLS)


def is_label_list_einsum_expression(expression: str) -> bool:
    return expression.startswith(LABEL_LIST_EINSUM_SENTINEL)
