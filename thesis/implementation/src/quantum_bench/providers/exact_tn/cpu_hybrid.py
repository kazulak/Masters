...
from quantum_bench.tn.slice_execution import execute_task_hybrid_slice_frontier_np_einsum
from quantum_bench.tn.slicing import build_slice_aware_taskgraph_model, validate_slice_aware_taskgraph_model


class CpuTnHybridSlicedFrontierExactRoute:
    name = "cpu_tn_hybrid_sliced_frontier_exact"
    backend_family = "cpu"
    identity = RouteIdentity(
        route_id=name,
        display_name="CPU exact tensor network hybrid sliced frontier diagnostic",
        role="internal_hybrid_diagnostic",
        simulation_method="exact_tensor_network",
        kernel_family="einsum_contraction",
        hardware_target="cpu",
        execution_mode="in_process_python_hybrid_sliced_frontier",
    ...
    def can_execute(self, graph: TaskGraph, context: BenchmarkContext) -> tuple[bool, str | None]:
        ...
        max_slice_count = int(options.get("max_slice_count", 2) or 2)
    ...
    def execute(self, prepared: object, context: BenchmarkContext) -> RouteResult:
        ...
        max_slice_count = int(payload.get("max_slice_count", 2) or 2)
