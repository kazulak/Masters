...
| QuEST CPU/GPU | A direct QuEST CPU adapter accepts six structurally verified `quest_compatible` families, validates the native state-dump contract, and records executable identity plus compute-only native timing/energy facts. Direct GPU runtime verification remains planned. | CPU adapter is software-tested with controlled processes; a real local runner was unavailable, so real CPU execution and GPU execution are not yet qualified on this branch. |
